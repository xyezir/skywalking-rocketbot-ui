...
<template>
  <div
    class="rk-performance-bar-select cp flex-h"
    v-clickout="
      () => {
        visible = false;
        search = '';
  ...
  >
    <div class="rk-performance-bar-i flex-h" @click="visible = !visible">
      <svg class="icon lg mr-15">
        <use :xlink:href="`#${icon}`"></use>
      </svg>
      <div class="mr-15 rk-performance-bar-i-text">
    ...
    <div class="rk-performance-sel" v-if="visible">
      <div>
        <input type="text" class="rk-performance-sel-search" v-model="search" />
        <svg class="icon sm close" @click="search = ''" v-if="search">
          <use xlink:href="#clear"></use>
        </svg>
      </div>
      <div class="rk-performance-opt-wrapper scroll_hide">
        <div
          class="rk-performance-opt ell"
...
<style lang="scss" scoped>
  ...
  .rk-performance-bar-i-text {
    max-width: 200px;
  }
  .rk-performance-bar-i {
    height: 100%;
    padding: 0 15px;
